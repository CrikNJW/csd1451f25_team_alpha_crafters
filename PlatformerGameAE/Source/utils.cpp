...
int IsAreaClicked(float area_center_x, float area_center_y, float area_width, float area_height, float click_x, float click_y) {
	...
	float right_bound = area_center_x + half_width; float top_bound = area_center_y - half_height;
...
int IsCircleClicked(float circle_center_x, float circle_center_y, float diameter, float click_x, float click_y) {
	float radius = diameter / 2.0; // distance = sgrt of [(X1 - X2)^2 + (Y1 - Y2)^2], we eliminate the sqrt by squaring the radius later
...
void UpdatePlayerMovement(Player *player , AEGfxVertexList* player_mesh) {
	player->speed = AEFrameRateControllerGetFrameTime() * 300.f; //speed of player according to frame rate //player movement 
